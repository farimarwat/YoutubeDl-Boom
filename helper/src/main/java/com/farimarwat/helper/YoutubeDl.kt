...
object YoutubeDl {
    ...
    fun getInProgressDownloadsCount(): Int {
        return try {
            ...
            val getInProgressDownloadsCountMethod = youtubeDLClass.memberFunctions.find { function ->
        ...
        return CoroutineScope(Dispatchers.IO + exception).launch {
            try {
                ...
                val params = mapOf(
                    ...
                    updateYoutubeDLMethod.parameters[4] to { throwable: Throwable -> onError(throwable) } // onError
                )
    ...
    suspend fun mapUpdateStatus(updateStatus: Any): String? {
        return withContext(Dispatchers.IO){
            try {
                val updateStatusClass = Class.forName("com.farimarwat.library.YoutubeDL\$UpdateStatus").kotlin
    ...
    suspend fun getUpdateChannel(value: String): Any {
        return withContext(Dispatchers.IO){
            try {
                val updateChannelClass = Class.forName("com.farimarwat.library.YoutubeDL\$UpdateChannel").kotlin
                ...
                val updateChannel = when (value) {
                    "STABLE" -> companionObject::class.memberProperties.find { it.name == "_STABLE" }?.call(companionObject)
                    "NIGHTLY" -> companionObject::class.memberProperties.find { it.name == "_NIGHTLY" }?.call(companionObject)
                    "MASTER" -> companionObject::class.memberProperties.find { it.name == "_MASTER" }?.call(companionObject)
    ...
    suspend fun version(appContext: Context?): String? {
        return withContext(Dispatchers.IO) {
            try {
                val youtubeDLUpdaterClass = Class.forName("com.farimarwat.library.YoutubeDLUpdater").kotlin
                val instance = youtubeDLUpdaterClass.objectInstance ?: youtubeDLUpdaterClass.createInstance()
